...
MQTTClient client = MQTTClient(10240);
...
void messageHandler(String &topic, String &payload) {
  ...
  } else if (topic == "PlantCare/" + String(AWS_IOT_THING_NAME) + "/requests") {
    Serial.println("INFO: The cloud has made a request");
...
void connectAWS()
{
  ...
  Serial.println("INFO: Subscribing to picture upload requests");
  client.subscribe("PlantCare/" + String(AWS_IOT_THING_NAME) + "/requests");
}
...
DynamicJsonDocument getComponentConfiguration () {
  ...
  int moisture = digitalRead(13);
  doc["moisture"] = moisture;

  return doc;
}

void processRequest (DynamicJsonDocument request) {
  Serial.println("INFO: Processing cloud request");

  if (request.containsKey("picture_upload")) {
    Serial.println("INFO: Request is about uploading a picture");
    uploadPicture();
  }
}
...
void updateShadow(DynamicJsonDocument delta) {
  ...
  if (delta.containsKey("flashLight")) {
    Serial.println("Requesting interaction with flash light");
    digitalWrite(4, LOW);
  ...
  status["picture_upload"] = delta["picture_upload"];
  reportStatus(status);
}
...
void uploadPicture () {
  Serial.println("Taking picture and uploading to cloud");
  ...
  Serial.println("INFO: Successfully taken picture");

  String imageBase64 = base64::encode(fb->buf, fb->len);
  esp_camera_fb_return(fb);
  
  Serial.println("INFO: Uploading taken picture");

  String response = "{ \"image\": \"" + imageBase64 + "\"}";
  client.publish("PlantCare/" + String(AWS_IOT_THING_NAME) + "/requests/picture_upload/response", response, false, 1);
  Serial.println("INFO: Image uploaded successfully");
}
...
void loop() {
  ...
  // If client is not connected to cloud, reconnect.
  if (!client.connected()) {
    Serial.println("ERROR: Connection lost to cloud");
    digitalWrite(4, HIGH);
    connectAWS();
    digitalWrite(4, LOW);
  }

  // Loop the mqtt client to not lose connectivity
  client.loop();

  // Take sensor measurements every 30 seconds
  if (it != 0 && it % 30000 == 0) {
    Serial.println("INFO: Reporting device status periodically");
    DynamicJsonDocument status = getComponentConfiguration();
    reportStatus(status);
  }
